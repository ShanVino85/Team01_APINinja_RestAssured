...
public class PatientModule extends RestUtils {
   ...
   //-----------------------------------Patient Post creation entering only by valid mandatory details(No pdf file added)--------------------------------------//
	...
	//-----------------------------------Patient PUT request with valid data--------------------------------------//
	...
	public void dietician_send_put_http_request_with_endpoint_with_valid_data_Add_pdf_file() throws IOException {
		...
		response = request.contentType("multipart/form-data").multiPart("patientInfo",PatientPutdata.ValiddataPUT() ,"application/json")
				  .multiPart("file", file2 ,"application/pdf").when().put(routes.getString("Put_UpdatePatient"));
		asString = response.then().log().all().extract().asString();
	...
	public void dietician_sent_put_http_request_with_invalid_endpoint() throws IOException {
		...
		response = request.contentType("multipart/form-data").multiPart("patientInfo",PatientPostdata.ExcelValiddata(),"application/json")
				  .multiPart("file", file1 ,"application/pdf").multiPart("vitals",PatientPutVitalsdata.AddPdfAddvitalsPUT(),"application/json")
				  .when().put(routes.getString("Put_invalidUpdatepatientwithvital/withoutvitals"));
	...
	public void dietician_send_put_http_request_with_endpoint_and_invalid_content_type() throws IOException {
		...
		response = request.contentType("application/pdf").multiPart("patientInfo",PatientPostdata.ExcelValiddata(),"application/json")
				  .multiPart("file", file1 ,"application/pdf").multiPart("vitals",PatientPutVitalsdata.AddPdfAddvitalsPUT(),"application/json")
				  .when().put(routes.getString("Put_Updatepatientwithvital/withoutvitals"));
	...
	public void admin_creates_put_request_by_entering_valid_data_into_the_form_data_key_and_value_fields() throws FileNotFoundException {
		request=given().spec(requestSpecification()).header("Authorization", "Bearer "+IdHolder.Admintoken).pathParam("patientId", IdHolder.patientId2);
	}
	...
	public void patient_creates_put_request_by_entering_valid_data_into_the_form_data_key_and_value_fields() throws FileNotFoundException {
		request=given().spec(requestSpecification()).header("Authorization", "Bearer "+IdHolder.Patienttoken).pathParam("patientId", IdHolder.patientId2);
	}
...
//	}
	...
	public void dietician_sends_put_http_request_to_the_endpoint_with_valid_patient_id() throws IOException {
		response = request.contentType("multipart/form-data").multiPart("patientInfo",PatientPostdata.ExcelonlyMandatorydata(),"application/json")
				  .multiPart("file", file2 ,"application/pdf").multiPart("vitals",PatientPutVitalsdata.ModifyvitalsPUT(),"application/json")
				  .when().put(routes.getString("Put_Updatepatientwithvital/withoutvitals"));
	}
	...
	//---------------GetPatientmorbidity Details withDietician Token Positive cases------------------------//
       ...
       String endpoint = routes.getString("Get_GetPatientsMorbidityDetails");
        ...
        List<PatientGetResponse> patients = response.jsonPath().getList("", PatientGetResponse.class);
          String fileId = patients.get(0).getFileId();
            ...
            logger.info("----------Dietician gets  morbidity details with file id -----------");
  }
	...
	public void dietician_create_get_request_patients_morbidity_details_by_endpoint_as_patient_file_id() {
          response = request.when().get(routes.getString("Get_RetrievePatientfilebyFileId").replace("{fileId}", IdHolder.fileId)).then().log().all().extract().response();
               logger.info("----------Dietician send  get  request  with file id -----------");
	}
	...
	//---------------GetPatientmorbidity Details with Patient Token Positive cases------------------------//
	...
	public void patient_send_get_http_request_with_retrieve_patients_morbidity_details_by_patient_endpoint_with_patient_token() {
		String endpoint = routes.getString("Get_GetPatientsMorbidityDetails");	
        response = request.when().get(endpoint).then().log().all().extract().response(); 
        logger.info("----------Patient send  get  request  Get_GetPatientsMorbidityDetails -----------");
	...
	//--------------------------Negative test cases GetPatientsMorbidityDetails--------------------//
	...
	public void dietician_create_get_request_with_no_auth_to_retrieve_patients_morbidity_details_by_patient_id() throws FileNotFoundException, IOException {
		request = given().spec(requestSpecification()).pathParam("patientId", IdHolder.patientId2);
	...
	public void admin_send_get_http_request_with_endpoint_to_retrieve_patients_morbidity_details_by_patient_id() {
		String endpoint = routes.getString("Get_GetPatientsMorbidityDetails");
        response = request.when().get(endpoint).then().log().all().extract().response();
        logger.info("----------Admin send  get  request to retrive morbidity details -----------");
	}
	...
	public void dietician_send_post_http_request_with_endpoint_to_retrieve_patients_morbidity_details_by_patient_id_with_invalid_method() {
        ...
        logger.info("----------Dietician send  invalid method -----------");
	}
	...
	public void dietician_send_get_http_request_with_endpoint_to_retrieve_patients_morbidity_details_by_invalid_patient_id() {
        ...
        response = request.when().get(endpoint).then().log().all().extract().response();
        logger.info("----------Dietician send  get request with invalid patient id -----------");
	}
	...
	public void dietician_send_get_http_request_with_invalid_endpoint_to_retrieve_patients_morbidity_details_by_patient_id_with_invalid_endpoint() {
        ...
        response = request.when().get(endpoint).then().log().all().extract().response();
        logger.info("----------Dietician send get request with  invalid endpoint -----------");
	}
	...
	//--------------------Negative test cases Get_RetrievePatientfilebyFileId---------------------------//
	...
	public void admin_send_get_http_request_with_endpoint_to_retrieve_patients_by_fiel_id() {
		response = request.when().get(routes.getString("Get_RetrievePatientfilebyFileId").replace("{fileId}", IdHolder.fileId)).then().log().all().extract().response();
		logger.info("----------Admin send get request with  patients by fielId -----------");
	}
	...
	public void dietician_send_post_http_request_with_endpoint_to_retrieve_patients_by_field_with_invalid_method() {
		response = request.when().post(routes.getString("Get_RetrievePatientfilebyFileId").replace("{fileId}", IdHolder.fileId)).then().log().all().extract().response();
		logger.info("----------Dietician send post  invalid method  -----------");
	...
	public void dietician_send_get_http_request_with_endpoint_to_retrieve_patients_by_invalid_file_id() {
		...
		response = request.when().get(routes.getString("InvalidFileid_Get_RetrievePatientfilebyFileId").replace("{fileId}", IdHolder.fileId.concat("002"))).then().log().all().extract().response();
		logger.info("----------Dietician send get request  invalid fileId  -----------");
	}
	...
	public void dietician_send_get_http_request_to_retrieve_patients_by_field_with_invalid_endpoint() {
		response = request.when().get(routes.getString("Invalidendpoint_Get_RetrievePatientfilebyFileId").replace("{fileId}", IdHolder.fileId)).then().log().all().extract().response();
		logger.info("----------Dietician send get request  invalid endpoint  -----------");
	}
